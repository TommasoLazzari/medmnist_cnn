...
def spatial_attention_block(input_feature, name_prefix="spatial_att"):
    #Computes a 2D spatial attention map using channel-wise pooling (Avg and Max) and a Conv2D layer, then applies it as a scale mask.
    #Channel-wise average pooling
    avg_pool = layers.Lambda(
        lambda x: __import__('tensorflow').reduce_mean(x, axis=-1, keepdims=True),name=f"{name_prefix}_avg"
    )(input_feature)
    #Channel-wise max pooling
    max_pool = layers.Lambda(
        lambda x: __import__('tensorflow').reduce_max(x, axis=-1, keepdims=True),name=f"{name_prefix}_max"
    )(input_feature)
    ...
    attention_map = layers.Conv2D(filters=1,kernel_size=7,padding="same",
        activation="sigmoid",use_bias=False,name=f"{name_prefix}_conv")(concat)
...
def create_attention_noaug_model(input_shape=(64, 64, 3), num_classes=8):
    ...
    #Injects a Spatial Attention Block after the convolutional feature extractor, refining spatial features before flattening.
